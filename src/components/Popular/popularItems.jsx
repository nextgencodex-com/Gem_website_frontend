...
const PopularItems = () => {
  ...
  useEffect(() => {
    if (productData) {
      ...
      setActiveImg(productData.image || productData.images?.[0]);
      setSize(productData.sizes?.[0] || "");
    } else {
      // Redirect back to popular if no product data
  ...
  const isInCart = product && cartItems.some(item => 
    item.id === `popular-${product.id}` || item.id === product.id
  );
    ...
    <div className="min-h-screen bg-black">
      ...
      <header className="px-6 md:px-12 py-6 relative z-10">
        ...
        <div className="mt-6 text-xs text-gray-500">
          Home / Popular / {product.name || product.title}
        </div>
      </header>
